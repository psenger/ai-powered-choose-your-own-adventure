...
REM Generate simple timestamp for backup filename
set "backup_filename=src-backup-%RANDOM%%RANDOM%.zip"
set "backup_path=%BACKUP_DIR%\%backup_filename%"

REM Create backup ZIP using PowerShell
echo Creating backup of current src directory...
powershell -command "Compress-Archive -Path '%SRC_DIR%\*' -DestinationPath '%backup_path%' -Force"
